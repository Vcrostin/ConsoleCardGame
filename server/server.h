...
private:
    ...
    void start_accept() {
        ...
        con_handler::pointer connection = con_handler::create(
                (boost::asio::io_context &) acceptor_.get_executor().context(), sessions);
    ...
    con_handler::DequeSessions sessions;
public:
//constructor for accepting connection from client
    explicit Server(boost::asio::io_service &io_service) : acceptor_(io_service, tcp::endpoint(tcp::v4(), 1234)) {
        sessions = std::make_shared<CustomDeque<Core::Session>>();
        start_accept();
    }

    void handle_accept(con_handler::pointer connection, const boost::system::error_code &err) {
        if (!err) {
//            con_handler::DequeSessions::iterator it;
//            {
//                std::lock_guard lock(dequeMutex);
//                if (sessions.empty() || sessions.back().IsBusy()) {
